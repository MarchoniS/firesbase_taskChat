...
class AssignedTasksPage extends StatefulWidget {
  ...
  const AssignedTasksPage({Key? key, required this.taskRepository})
      : super(key: key);
...
class _AssignedTasksPageState extends State<AssignedTasksPage> {
  ...
  void _applyFilters() {
    ...
    setState(() {
      filteredTasks = allTasks.where((task) {
        ...
        final matchesAssigner = selectedAssigner == null ||
            task.assignedByUserId == selectedAssigner;
          ...
          builder: (context, snapshot) {
            ...
            if (allTasks.isEmpty) {
              ...
              _fetchAssignerNames(assignerIds).then((names) {
                assignerNames = names;
                _applyFilters(); // ✅ Safe to call here
              });
            }

            final completedTasks = filteredTasks.where((t) =>
            t.status == 'completed').toList();
            final pendingTasks = filteredTasks.where((t) =>
            t.status != 'completed').toList();
                          ...
                          items: [
                            ...
                            ...assignerIds.map((id) {
                              return DropdownMenuItem<String>(
                                value: id,
                                child: Text(
                                    assignerNames[id] ?? "Assigner ID: $id"),
                              );
                            }).toList(),
                          ],
                          onChanged: (value) {
  ...
  Widget _buildTaskCard(AssignedTask task) {
    ...
    final assignerName = assignerNames[task.assignedByUserId] ??
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            Text(
              'Assigned on: ${DateFormat('dd MMM yyyy').format(
                  task.createdAt)}',
              style: const TextStyle(fontSize: 12),
            ),
          ],
        ...
        trailing: isCompleted
          ...
          onPressed: () async {
            ...
            if (confirmed == true) {
              await widget.taskRepository.updateTaskStatus(
                  task.id, 'completed');
              _refreshTasks();
            }
          },
